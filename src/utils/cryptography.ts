...
const bytIV = Buffer.from([
  121, 241, 10, 1, 132, 74, 11, 39, 255, 91, 45, 78, 14, 211, 22, 62
])
...
const EncryptString128Bit = (vstrTextToBeEncrypted: string): string => {
  try {
    ...
    const intLength = Buffer.byteLength(vstrEncryptionKey, "ascii")
    if (intLength >= 32) {
    ...
    const bytEncoded = Buffer.concat(encryptedChunks)
...
const DecryptString128Bit = (vstrStringToBeDecrypted: string): string => {
  try {
    const vstrDecryptionKey: string = KeyEncript
    const bytDataToBeDecrypted = Buffer.from(vstrStringToBeDecrypted, "base64")
    const objRijndaelManaged = crypto.createDecipheriv(
      "aes-256-cbc",
      padKey(vstrDecryptionKey),
      bytIV
    )
    ...
    const decrypted = Buffer.concat(decryptedChunks).toString("ascii")
